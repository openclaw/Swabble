...
const productDescription =
  "A Swift wake-word daemon that listens locally with Apple Speech.framework, strips the wake word, and runs your shell hook with the transcript.";
const installCommand = "swift run swabble setup";

const sections = [
  ["Start", ["index.md", "install.md", "quickstart.md"]],
  ["Operate", ["configuration.md", "cli.md", "hooks.md"]],
  ["Build", ["library.md", "development.md"]],
  ["Reference", ["spec.md"]],
];
...
function homeHero(page) {
  ...
  const quickstartRel = pageMap.get("quickstart.md")?.outRel
    ...
    : "quickstart.html";
  const services = ["macOS 26", "Swift 6.2", "On-device speech", "Wake word", "Shell hooks", "SRT export"];
  return `<header class="home-hero">
        <p class="eyebrow">Speech.framework · Local hooks</p>
        ...
        <div class="home-cta">
          ...
          <div class="home-install" aria-label="Setup command">
    ...
    <aside class="sidebar">
      <div class="sidebar-head">
        <a class="brand" href="${hrefToOutRel("index.html", page.outRel)}" aria-label="${productName} docs home">
          ...
          <span><strong>${escapeHtml(productName)}</strong><small>speech hook docs</small></span>
        </a>
        ${themeToggleHtml()}
      </div>
      <label class="search"><span>Search</span><input id="doc-search" type="search" placeholder="wake word, hook, transcribe"></label>
...
function navTitle(page) {
  ...
  return page.title.replace(/^`swabble\s*/, "").replace(/`$/, "");
}
...
function highlightShellLine(line) {
  ...
  working = working.replace(/\b(swift|swabble|brew|git|gh|make|sudo|cd|export|cat|curl|jq|ls|mv|cp|rm|mkdir|docker|tail|node|npm|pnpm|yarn)\b/g, (m) => stashAdd(m, "hl-cmd"));
